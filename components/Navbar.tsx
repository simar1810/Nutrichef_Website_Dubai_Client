      ...
      {open ? (
        <>
          ...
          <div
            ...
            className="fixed right-0 z-[60] flex w-full max-w-[min(100vw,20rem)] flex-col overflow-hidden rounded-l-2xl border-l border-t border-border-subtle bg-surface shadow-[0_24px_64px_-16px_rgba(27,48,34,0.18)] lg:hidden sm:max-w-[19rem]"
            style={{
              top: DRAWER_TOP,
              height:
          ...
          <div className="hidden items-center gap-1 lg:flex">
            {navLinks.map(({ href, label }) => (
              <Link
                key={href}
                href={href}
                className="rounded-full px-3.5 py-2 text-sm font-medium text-foreground/65 transition hover:bg-bg-light hover:text-foreground"
              >
                {label}
              </Link>
            ))}
